class FornecedoresController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @fornecedor.save
        format.html { redirect_to @fornecedor, notice: t('.sucesso') }
  ...
  def update
    respond_to do |format|
      if @fornecedor.update(fornecedor_params)
        format.html { redirect_to @fornecedor, notice: t('.atualizado') }
  ...
  def destroy
    @fornecedor.destroy
    respond_to do |format|
      if @fornecedor.entradas.size > 0
        format.html { redirect_to fornecedores_url, danger: 'Imposivel excluir este fornecedor. Ela está registrada em entradas !' }
        format.json { head :no_content }
      else
        format.html { redirect_to fornecedores_url, notice: t('.apagado') }
        format.json { head :no_content }
      end
    end
  end
  ...
  private
    ...
    def fornecedor_params
      params.require(:fornecedor).permit(:nome, :cnpj, :endereco, :telefone, :contato)
    end
end
