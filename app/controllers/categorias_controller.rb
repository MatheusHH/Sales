class CategoriasController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @categoria.save
        format.html { redirect_to @categoria, notice: t('.sucesso') }
  ...
  def update
    respond_to do |format|
      if @categoria.update(categoria_params)
        format.html { redirect_to @categoria, notice: t('.atualizado') }
  ...
  def destroy
    ...
    respond_to do |format|
      if @categoria.produtos.size > 0
        format.html { redirect_to categorias_url, danger: 'Imposivel excluir esta categoria. Ela está registrada em produtos !' }
        format.json { head :no_content }
      else
        format.html { redirect_to categorias_url, notice: t('.apagado') }
        format.json { head :no_content }
      end
    end
  end
