class ModalidadesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @modalidade.save
        format.html { redirect_to @modalidade, notice: t('.sucesso') }
  ...
  def update
    respond_to do |format|
      if @modalidade.update(modalidade_params)
        format.html { redirect_to @modalidade, notice: t('.atualizado') }
  ...
  def destroy
    @modalidade.destroy
    respond_to do |format|
      format.html { redirect_to modalidades_url, notice: t('.apagado') }
  ...
  private
    ...
    def modalidade_params
      params.require(:modalidade).permit(:nome, :valor)
    end
end
